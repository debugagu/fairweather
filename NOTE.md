...
Same note is readable on the live site at `/note`.
...
pleasant afternoon if you are wandering between air-conditioned shops and a
genuine problem four hours into a walk. Advice that tries to serve both serves
neither, so the app says on its own front page that if you are going to a
wedding, you should ignore it.
...
Eleven daily fields come back plus two hourly series.

The date inputs are bounded at both ends — no earlier than today, no later than
fourteen days out — and anything the browser lets through anyway is caught again
in JavaScript before a request is made, because a native date picker is a
suggestion, not a guarantee. Past dates are refused rather than quietly
...
deduplicated twice: once by item, and once by redundancy — a waterproof shell is
already windproof, and three litres of water capacity already covers two.
...
| Heat | 28° | 32° | 38° | Apparent temperature, not the thermometer — what matters is the load on someone already working. 32° is where a full day of exertion starts costing you. 38° is not a judgement call. |
...
| Daylight | <9.5h | — | — | Capped at level 1 deliberately. Short days never make weather bad, but they quietly decide how long a route can be, and no forecast app reports it usefully. |
...
listed — at that point the sentence stops being a verdict and becomes a weather
report, which is the thing this product exists to avoid.
...
**Cloud cover.** It is the field almost every weather app leads with, and it is
close to irrelevant to whether a day on foot is good — an overcast 19° day is
excellent walking. Sunshine hours would have made the interface look richer and
the advice worse, so the app never fetches the field at all.

Two smaller omissions follow the same logic. Humidity is not shown as its own
number, because apparent temperature already contains it in the only form that
matters — how hard the day will feel. And the packing list is weather-driven
only: no boots, no map, no first aid. Padding it with things you would take
anyway would make it longer and less trustworthy.
...
3. **A second audience, properly.** Not a toggle bolted onto these numbers, but
   a separate table and a separate voice. The moment this app tries to advise the
...
Plain HTML, CSS and JavaScript. Four small ES modules, no framework, no build
step, no dependencies. Two typefaces that ship with the operating system —
Garamond for anything that is a judgement, Trebuchet MS for anything that is a
number — and five colours, one of which has a job. Deployed as static files on
Vercel.
...
I used Claude Code as a pair while building it. The product decisions — the
audience, the six concerns, where each threshold sits, what to leave out — are
mine, and every threshold lives in one file (`js/verdict.js`) so they can be
argued with.
